...
std::unique_ptr<std::vector<std::shared_ptr<Test>>> make(){
    std::unique_ptr<std::vector<std::shared_ptr<Test>>> uptr = std::make_unique<std::vector<std::shared_ptr<Test>>>();
    return uptr;
}
...
void display(const std::vector<std::shared_ptr<Test>>&vec){
    ...
    for(const auto shptr: vec){
        std::cout << shptr->get_data() << std::endl;
    }
}